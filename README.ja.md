...
| `O` | リンクをブラウザで開く |
| `B` | ウィンドウを背面へ移動。背面のままフォーカスして操作できる。`B` または `Esc` で解除して前面へ戻る |
| `F` / `F11` | 全画面 |
| `L` | UI 言語を切り替え |
| `Esc` | 背面指定を解除して前面へ戻る。全画面なら解除、通常表示なら Gallery へ戻る |
...
| GPU | wgpu が選んだアダプタの名前 |
| 描画経路 | `Vulkan · DiscreteGpu` のような形。どの API とどの種類の GPU か |

重い作品がどこで重いかは、上の 4 つで分かる。命令数が多くて三角形が少なければ
言語側に無理をさせている。逆なら描画側。

下 2 行は作品ごとに変わらないので、いちばん下に置いている。内蔵と外付けの
2 枚が載っている機械では、どちらが選ばれたかで速さが何倍も変わる。遅いという
報告を受けたとき最初に見る場所で、同じ内容は起動時のログにも残る
(ドライバの版まで含めて `RUST_LOG=info` で見える)。

### 全画面でのカーソル

全画面で 3 秒操作が途切れると、マウスカーソルが消える。動かせば戻る。矢印が
居座ると作品の一部のように見えてしまうため。

オーバーレイ (2.6 秒) より少し遅らせている。同時に消すと、画面から 2 つのものが
一度に消えて何が起きたのか読み取りにくい。

**カーソルがこの窓の上にないときは消さない。** 全画面でもモニタが 2 枚あれば
カーソルは別の画面へ出ていける。そこで消すと、こちらを見ていない人のカーソルを
奪うことになる。

消すのは `window.set_cursor_visible(false)` ではなく、egui へ
`CursorIcon::None` を要求する形で行う。直接呼ぶと egui-winit が毎フレーム
`set_cursor_visible(true)` を呼び返すので、取り合いになる。
